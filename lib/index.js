...
generateUrl = function (options) {
    ...
    if (options.username && options.password) {
        baseUrl = 'postgres://' + options.username + ':' + options.password + '@' + host + ':' + port + '/' + database
    } else {
        baseUrl =  'postgres://' + host + ':' + port + '/' + database
    }

    //Now, thanks to options, we may add some query params
    let url = new URL(baseUrl);
    if(options.ssl != null) url.searchParams.set("ssl",options.ssl);
    if(options.types) url.searchParams.set("types",options.types);
    if(options.statement_timeout) url.searchParams.set("statement_timeout",options.statement_timeout);
    return url.toString()
}
...
PostgresStorage = (function () {
    function PostgresStorage (settings1) {
        ...
        if (!this.settings.url) {
            this.settings.connectionString = generateUrl(this.settings);
        }
        else
        {
            this.settings.connectionString = this.settings.url;
        }
    ...
    PostgresStorage.prototype.connect = function (callback) {
        ...
        if (this.db) {
            ...
            self.db = new pg.Client(self.settings.connectionString)
             return self.db.connect(function (err) {
                debug('Postgres connection established: ' + self.settings.connectionString)
                if (callback) {
    ...
    PostgresStorage.prototype.getAllFilesById = function (id, callback) {
        let query = 'select * from ' + this.settings.table
        if (id && !Array.isArray(id)) id = [id]
        if (id) query = query + ' where id IN (' + id.join(',') + ')'
        return this.db.query(query, function (err, res) {
            if (err) {
                return callback(err)
            }
            if (!res || !res.rows) {
                err = new Error('Error when retrieving Files')
                err.status = 500
                return callback(err)
            }
...
PostgresStorage.prototype.getAllFilesById.shared = true

PostgresStorage.prototype.getAllFilesById.shared = true
PostgresStorage.prototype.getAllFilesById.accepts = [
    {
        arg: 'id',
        type: 'number'
    }
...
PostgresStorage.prototype.getAllFilesById.returns = {
    arg: 'files',
    type: 'files',
    root: true
}

PostgresStorage.prototype.getAllFilesById.http = {
    verb: 'get',
    path: '/'
}
